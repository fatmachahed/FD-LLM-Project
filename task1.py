...
def analyze_fds(fds):
    ...
    # Number of FDs
    total = len(fds)
    lines.append(f"Total FDs: {total}")

    # Average LHS size
    lhs_sizes = [len(lhs) for lhs, _ in fds]
    lines.append(f"Average LHS size: {round(np.mean(lhs_sizes), 2)}")
    ...
    for k, v in single.items():
        if len(v) >= 3:
            lines.append(f"  {k} -> {len(v)} attributes")
    ...
    for lhs, rhs in fds:
        if len(lhs) == 1 and lhs[0].lower() in ["id", "index"]:
            lines.append(f"ID-based : {lhs[0]} → {rhs}")

        elif len(lhs) >= 4:
            lines.append(f"LHS très large : {', '.join(lhs)} → {rhs}")

        elif rhs in lhs:
            lines.append(f"Dégénérée : {', '.join(lhs)} → {rhs}")
